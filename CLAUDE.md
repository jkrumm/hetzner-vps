...
**imgproxy** — image CDN at `img.DOMAIN` (public, proxied through Cloudflare so the edge is the cache layer). Renders on-the-fly resizes/format conversions from the **`img/` prefix of the existing backups bucket**; the bucket stays private and is never served directly. URLs are **unsigned** (`/insecure/` prefix) — access control is the prefix lock plus non-enumerable object keys, not the URL. Stateless: no volumes, no DB.
